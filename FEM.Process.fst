...
/// Check if a parent term is effectful. This is used to look for instances of ``HS.mem``
/// to use to instantiate pre/postconditions, which means that the term should
/// be a "parent" term of the term under study, as generated by ``explore_term``
...
/// consider.
let parent_term_is_effectul dbg ge tv : Tac bool =
  let is_effectful tm =
...
/// Look for a term of the form ``let h = ST.get ()`` in a list of parent terms
/// and return the let-bound bv. Abort the search if we find a non-effectful term.
val find_mem_in_parents:
     dbg:bool
  -> ge:genv
  -> decls:list term_view
  -> Tac (option bv)

let rec find_mem_in_parents dbg ge parents =
  match parents with
  | [] -> None
  | tv :: parents' ->
    ...
    | Some bv -> Some bv
    | None ->
      if parent_term_is_effectul dbg ge tv then None
      else find_mem_in_parents dbg ge parents'
...
val find_equality_from_post : bool -> genv -> term -> bv -> term -> typ -> effect_info -> Tac (genv & option term)
let find_equality_from_post dbg ge0 tm let_bv ret_value ret_type einfo =
...
val find_context_equality : bool -> genv -> term -> list term_view -> Tac (genv & option term)
let rec find_context_equality dbg ge0 tm parents =
  match parents with
  | [] -> ge0, None
  | tv :: parents' ->
    ...
    | Tv_Let _ _ bv' def _ ->
      ...
      begin match find_equality_from_post dbg ge0 tm bv' ret_value ret_typ einfo with
      | ge1, Some p -> ge1, Some p
      | _, None -> find_context_equality dbg ge0 tm parents'
      end
   | _ -> find_context_equality dbg ge0 tm parents'
